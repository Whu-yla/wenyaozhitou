...
async function init() {
    ...
    renderTrends();
    renderComp();
    loadBookmarksFromServer();  // 🚀 从服务端恢复书签
    restoreFilters();  // 恢复上次的筛选条件
    doFilter();
    } catch(e) {
        console.error('init failed:', e);
        document.getElementById('tBidTb').innerHTML = '<tr><td colspan="10" style="text-align:center;color:#ef4444;padding:20px">⚠️ 数据加载失败，请刷新页面重试</td></tr>';
    }
}
...
function renderComp() {
    ...
    if (!competitors.categories || !competitors.categories.length) {
        ...
        document.getElementById("compCat").innerHTML = competitors.categories.map(c =>
            '<div class="comp-item"><span>' + esc(c[0]) + '</span><span style="color:#60a5fa;font-weight:600">' + c[1] + '次</span></div>'
        ).join('');
    }
    // 竞品排行
    if (!competitors.competitors || !competitors.competitors.length) {
        ...
        document.getElementById("compTop").innerHTML = competitors.competitors.slice(0, 10).map((c, i) =>
            '<div class="comp-item">' +
            '<span>' + (i + 1) + '. ' + esc(c.name || '').substring(0, 20) + '</span>' +
            '<span class="cat">' + esc(c.type || '') + '</span>' +
            '<span style="color:#f59e0b">' + c.wins + '次</span>' +
            (c.amount_display ? '<span style="color:#64748b;font-size:10px">' + c.amount_display + '</span>' : '') +
            '</div>'
        ).join('');
    }
    // 大项目
...
function doFilter() {
    ...
    tb.innerHTML = page.map((i, idx) => {
        ...
        const titleFull = esc(i.title || "");
        const titleShort = titleFull.substring(0, 48);
        const linkUrl = url_fix(i.url);
        if (tab === "bid") {
            return '<tr><td style="text-align:center"><input type="checkbox" class="row-check" data-id="' + i.id + '" onchange="onCheckChange(\'' + chkAllId + '\')"></td>' +
                ...
                '</td><td title="' + titleFull + '">' + star + '<a href="' + linkUrl + '" target="_blank" style="color:inherit;text-decoration:none">' + titleShort + '</a>' + newBadge +
                '</td><td class="hide-mobile">' + cat +
                '</td><td class="hide-mobile">' + (i.procurement_owner ? esc(i.procurement_owner.substring(0, 20)) : '—') +
            ...
            return '<tr><td style="text-align:center"><input type="checkbox" class="row-check" data-id="' + i.id + '" onchange="onCheckChange(\'' + chkAllId + '\')"></td>' +
                '<td style="text-align:center;color:#475569;font-size:10px">' + (st + idx + 1) + '</td>' +
                '<td class="' + cl + '">' + sc.toFixed(1) +
                '</td><td title="' + titleFull + '">' + star + '<a href="' + linkUrl + '" target="_blank" style="color:inherit;text-decoration:none">' + titleShort.substring(0, 43) + '</a>' + newBadge +
...
function toggleStar(id) {
    ...
    syncBookmarksToServer(stars);  // 🔗 同步到服务端
    doFilter();
}
...
// 🔗 服务端书签同步
let _syncTimer = null;
function syncBookmarksToServer(stars) {
    clearTimeout(_syncTimer);
    _syncTimer = setTimeout(() => {
        fetch(API_BASE, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({bookmarks: stars})
        }).then(r => r.json()).then(d => {
            console.log('[书签同步]', d.count, '条已同步到服务端');
...
function loadBookmarksFromServer() {
    fetch(API_BASE).then(r => r.json()).then(d => {
        if (d.bookmarks && d.bookmarks.length > 0) {
            ...
            const merged = [...new Set([...local, ...d.bookmarks])];
            localStorage.setItem(STARS_KEY, JSON.stringify(merged));
            console.log('[书签恢复] 服务端' + d.bookmarks.length + '条 + 本地' + local.length + '条 → 合并' + merged.length + '条');
            doFilter();
        }
    }).catch(e => console.warn('[书签恢复失败]', e));
}
