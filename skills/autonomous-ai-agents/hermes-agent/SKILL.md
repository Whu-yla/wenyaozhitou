...
### Cross-Platform File-Editing Pitfall (critical)

**PITFALL:** `execute_code`'s `hermes_tools.read_file()` returns content with `LINE_NUM|`
prefixes on every line. **Never** pass that output directly to `write_file` — it embeds
line numbers into source files, causing syntax errors on every subsequent run.

**Safe patterns:**
- Use the `patch` tool for targeted edits (reads files directly, no prefixes)
- Use `terminal(command="python3 -c '...'")` for bulk replacements
- If you must use `execute_code`, strip line numbers before writing: `re.sub(r'^\d+\|', '', content, flags=re.MULTILINE)`

Full recovery recipe: `skill_view(name="hermes-agent", file_path="references/execute-code-pitfalls.md")`.
...
For the full SKILL.md authoring guide — frontmatter requirements, validator constraints, peer-matched structure, and common pitfalls — see `references/skill-authoring.md`. Quick rules:
- Skills live at `skills/<category>/<name>/SKILL.md` in the repo tree.
- Use `write_file` for in-repo creation; `skill_manage(action='create')` targets `~/.hermes/skills/`.
- `name` and `description` (≤ 1024 chars) are required; `version`/`author`/`license`/`metadata` are peer-expected.
- Total file ≤ 100,000 chars; aim for 8-14k, split into `references/` for larger content.
