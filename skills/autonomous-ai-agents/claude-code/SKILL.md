...
13. **Anthropic-only — no custom providers.** Claude Code ONLY works with Anthropic's API (plus Bedrock/Vertex/Foundry). It does NOT support custom OpenAI-compatible endpoints like DeepSeek, Ollama, or LiteLLM. For non-Anthropic providers, use OpenCode (`opencode` skill) instead — it supports any OpenAI-compatible API including DeepSeek.
